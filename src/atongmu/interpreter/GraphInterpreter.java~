...
public final class GraphInterpreter extends Interpreter{
	...
	public GraphInterpreter(String file, a2f t, String label){
		dotfile = file;
		translator = t;
		graphlabel=label;
		try{
	...
	private void interpretObjects(){
		...
		for (int i=0;i<nodes.size();i++){
			...
			color=Color.ORANGE;
			drawObjects(node.getName(),node.getType().getName(),color,"");
